 ...
 * | `site/index.ts` | the renderer, the lazy charts and the interactions | `test/index-page.test.ts` + the browser run |
 *
 ...
 *
 * Enforced indirectly, the same way `site/crashes-view.ts` is:
 ...
 * class or a glyph — those are `site/index.ts`'s, per the seam rule.
 */
 ...
 * that look like a real empty series. Divergence 4 in `site/index.ts`.
 */
 ...
 * hole silently biasing a headline number. `site/index.ts` logs a console
 ...
 *    label the window, and `site/index.ts` does exactly that: it reads
 ...
 * the difference is stated here and in `site/index.ts`'s header because a
