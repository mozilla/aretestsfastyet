 ...
 *
 * This is **page-local**. It sits in `site/` beside the renderer that consumes
 ...
 *  3. **The renderer becomes mechanical.** `site/crash-viewer.ts` is a
 ...
 * directory, and a node test imports `site/crash-view.ts` exactly as easily as
 ...
 * in the page directory, and `tsconfig.site.json` gives it the DOM lib, so that
 ...
 * as such in `site/crash-viewer.ts`'s divergences.
 *
