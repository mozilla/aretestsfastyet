 ...
 * values, with no DOM.
 *
 * ## Why this page does not use `next/drilldown-view.ts`
 *
 ...
 *
 * What *is* reused: `next/drilldown-render.ts`'s DOM primitives (`el`,
 ...
 * page-independent — see `next/errors.ts`.
 *
 ...
 *
 * Enforced the same way as `next/drilldown-view.ts`: `test/errors-view.test.ts`
 ...
 * Reproduced; see divergence list in `next/errors.ts`.
 */
...
export function prepareErrors(raw: ErrorsFile): PreparedErrors {
    ...
    // See `next/errors.ts`'s divergence list, entry 6.
...
export interface SubGroup {
     ...
     * cannot use `next/drilldown-view.ts`'s fixed `PathNode` → `TestNode`.
     */
    key: number;
    count: number;
 ...
 * `next/drilldown-view.ts` does: the shared version **mutates** its input,
 ...
 * `next/drilldown-view.ts`'s `isHistoricalDate`, where an absent date *also*
