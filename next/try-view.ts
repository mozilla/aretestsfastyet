 ...
 *
 * `tsconfig.next.json` gives `next/` the DOM lib, so that is a discipline rather
 ...
 *
 * The shape `next/try-worker.ts` posts back, plus the three fields the main
 ...
 * `next/try.ts` passes this array straight to `fetchFlakinessData`, which
 ...
 * (`try.html:1117`, transliterated at `next/try.ts:623`) appends each profile
 ...
 * them. It is read verbatim by the renderer (`next/try.ts`, which assigns it to
 ...
 * bundle should not pull in the link builders; see `next/try-worker.ts`.
 */
