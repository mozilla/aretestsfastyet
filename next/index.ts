 ...
 * | `next/index-view.ts` | this page's windows, merge, rows and traces | `test/index-view.test.ts`, no DOM |
 ...
 * table keeps the page's arithmetic in `next/index-view.ts` and
 ...
 * so.
 *
 * `next/index-view.ts`'s header carries the table. What this file does about it
 ...
 *     markup now throw once each.** The same arrangement `next/crashes.html`,
 *     `next/failures.html` and `next/errors.html` already ship, for the same
 ...
 * reason `next/issues.ts:310-320` records for Chart.js: `tsconfig.next.json`
 * compiles all of `next/**` as one program, so a second global declaration of
...
/**
 * Builds an element. The same helper `next/drilldown-render.ts` exports, and it
 ...
 *
 * **Rounded once, here, from the raw ratio.** `next/index-view.ts` carries
    ...
    if (unlisted.length > 0) {
        console.warn(
            `${harness}: the data has flavors this page does not show: ${unlisted.join(', ')}. ` +
                'Add them to MOCHITEST_FLAVORS in next/index-view.ts.'
        );
    }
 ...
 * Starts the page.
 *
 * Exported and called from `next/index-main.ts` rather than run on import, so a
 * test can import this module without attaching listeners and fetching data —
 * the property `next/crashes-main.ts` documents and the reason 2,598 lines of
