 ...
 * boundary `next/crash-view.ts` settled:
 *
 ...
 * all plain values a node test can assert on, and `next/test.ts` becomes a
 * transliteration.
 *
 ...
 *
 * `tsconfig.next.json` gives `next/` the DOM lib, so that is a discipline
