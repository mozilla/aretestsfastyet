 ...
 * `next/drilldown-controller.ts`, which this page and the crashes page share. A
...
/**
 * This page's state and interactions. See `next/crashes.ts` for why the
 ...
 * map: `next/crashes.ts` returns the whole unfiltered tree here, so its rows
 ...
 *
 * Deliberately **not** shared with `next/crashes.ts`, which has the same shape
 ...
 * that page's own suite. See the table in `next/crashes.ts`'s copy for the full
 ...
 * `next/crashes.ts`'s copy: the only line that differs between the two pages is
...
const SPEC: PageSpec = {
    ...
    // than cosmetic. `next/crashes.ts` reproduces upstream and does nothing.
