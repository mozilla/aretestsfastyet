...
// Repository holding this site's sources, linked from the "source" button.
const SOURCE_REPO_URL = 'https://github.com/mozilla/aretestsfastyet';

// The GitHub button: opens the repository this site is built from. Shown on
// every page that loads this script, next to the dashboard menu when there is
// one.
function createSourceButton() {
    const link = document.createElement('a');
    link.className = 'dash-source';
    link.href = SOURCE_REPO_URL;
    link.target = '_blank';
    link.rel = 'noopener';
    link.title = 'View this site’s source on GitHub';
    link.setAttribute('aria-label', 'View this site’s source on GitHub');
    link.innerHTML = `
...
// Inject the top-right nav: the GitHub source button (on every page) plus the
// hover menu (on pages that don't already list the dashboards inline).
function renderDashboardNav() {
    const nav = document.createElement('div');
    nav.className = 'dash-nav';
    if (shouldAutoInjectMenu()) nav.appendChild(renderDashboardMenu());
    nav.appendChild(createSourceButton());
